function [modslope, levels, log2spec ] = complexwaveletspectramodulus(data, L, wf, k1, k2, ismean)
%
...
%phasewddata = phasewddata + pi + 0.1;
y = [];
for i =  L:(lnn-1)
   help = modwddata(   round(2^(i)+1) : round(2^(i+1))   )   ;
   if  ismean == 1
       y = [y mean(help.^2)];
   elseif ismean == 0
       y = [y median(help.^2)];
